...
List<MatchDetection> matchDetections;
    ...
    void DetectMatch(Block block)
    {
        ...
        int left = block.X;
        while (left > 0 && board.Blocks[left - 1, block.Y].State == Block.BlockState.Idle && board.Blocks[left - 1, block.Y].Type == block.Type)
        {
            left--;
        }
        
        int right = block.X + 1;
        while (right < Board.Columns && board.Blocks[right, block.Y].State == Block.BlockState.Idle && board.Blocks[right, block.Y].Type == block.Type)
        {
            right++;
        }
        
        int bottom = block.Y;
        while (bottom > 0 && board.Blocks[block.X, bottom - 1].State == Block.BlockState.Idle && board.Blocks[block.X, bottom - 1].Type == block.Type)
        {
            bottom--;
        }
        
        int top = block.Y + 1; // Exclude the top row since it's for new incoming blocks
        while (top < Board.Rows - 1 && board.Blocks[block.X, top].State == Block.BlockState.Idle && board.Blocks[block.X, top].Type == block.Type)
        {
            top++;
        }
